...
import { distance2d, fileOpen, resolvablePromise, withBatchedUpdates, withBatchedUpdatesThrottled, type ResolvablePromise } from "./utils";
...
export default function ExampleApp({
  ...
  const [theme, setTheme] = useState<Theme>(isDarkMode ?? false ? "dark" : "light");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const stateAndContent = JSON.parse(jsonData || "{}");
      initialStatePromiseRef.current.promise.resolve({
        appState: stateAndContent.appState,
        elements: stateAndContent.elements,
      });
    };
    fetchData();
  }, [excalidrawAPI, convertToExcalidrawElements, MIME_TYPES]);

  const closeExcalidrawBtn = <button
      onClick={onClose}
      id="excalidraw-close-button"
    >
  ...
  return (
    <div className="excalidraw-wrapper" style={{ height: `${height}px`, position: "relative", overflow: "hidden" }}>
